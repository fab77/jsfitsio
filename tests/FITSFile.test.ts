...
describe("FITSFile data model", () => {
  ...
  test("contains a primary HDU", () => {
    ...
    const payload = new Uint8Array([0xff, 0xf6, 0x00, 0x14]);

    const typedData = new Int16Array([-10, 20]);
    ...
    const file = new FITSFile([primary]);
  ...
  test("primary HDU is also an ImageHDU", () => {
    ...
    expect(primary instanceof ImageHDU).toBe(true);
  });
  ...
  test("stores payload metadata", () => {
    ...
    expect(primary.bitpix).toBe(16);

    expect(primary.shape).toEqual([2, 1]);

    expect(primary.naxis).toBe(2);

    expect(primary.elementCount).toBe(2);

    expect(primary.typedData).toBe(typedData);
  });
  ...
  test("supports a 3D image cube", () => {
    const typedData = new Float32Array([1, 2, 3, 4, 5, 6, 7, 8]);

    const primary = new PrimaryHDU(
      new FITSHeaderManager(),
      new Uint8Array(typedData.byteLength),
      2880,
    ...
    expect(primary.naxis).toBe(3);

    expect(primary.shape).toEqual([2, 2, 2]);

    expect(primary.elementCount).toBe(8);

    expect(primary.bitpix).toBe(-32);

    expect(primary.typedData).toBeInstanceOf(Float32Array);
  });
});
